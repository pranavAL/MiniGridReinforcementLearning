...
class MiniGridEnv(gym.Env):
    ...
    def _gen_grid(self, width, height):
        ...
        pos1 = (4, self._rand_int(1, height-1))
        self.grid.set(*pos1, None)
        self.grid.vert_wall(8, 0)
        pos2 = (8, self._rand_int(1, height-1))
        self.grid.set(*pos2, None)

        self.obstacles = []
        for i in range(self.path1_obstacles):
            self.obstacles.append(Ball())
            self.place_obj(self.obstacles[i], top=(3,0), size=(1,9), max_tries=100)

        for i in range(len(self.obstacles), self.path1_obstacles + self.path2_obstacles):
            self.obstacles.append(Ball())
            self.place_obj(self.obstacles[i], top=(7,0), size=(1,9), max_tries=100)
